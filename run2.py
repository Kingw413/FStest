...
import pandas as pd
import csv
STRATEGY_VALUES = ['vndn', 'lsif', 'mupf', 'prfs', 'mine']
# RESULTS_FILES = ['fip.csv', 'fdp.csv', 'delay.csv' , 'isr.csv']
RESULTS_VALUES = ['FIP', 'FDP', 'ISD' , 'ISR']
TIME=60
def calMetric(logfile: str, delayfile: str, num=100, rate=1.0): 
    logs = open(logfile, 'r').readlines()
    if (logs[-1] != "end" and os.path.getsize(delayfile)==0):
        return [pd.NA]*5
...
def writeMetricToFile(folder_path: str, metrics_strategy: pd.DataFrame, run_time: int):
    ...
    file_path = os.path.join(folder_path, f'run{run_time}')
    try:
        df = pd.read_csv(file_path, index_col=0)
    except FileNotFoundError:
        df = pd.DataFrame(index=RESULTS_VALUES, columns=[])
        df.to_csv(file_path)
    df = pd.concat([df, metrics_strategy])
    df.to_csv(file_path)

    # for i in range(len(RESULTS_FILES)):
    #     file = os.path.join(folder_path, RESULTS_FILES[i])
    #     with open(file, 'a', newline='') as csvfile:
    #         csvwriter = csv.writer(csvfile)
    #         # 如果文件是空的，先写入表头
    #         if csvfile.tell() == 0:
    #             csvwriter.writerow([f'{STRATEGY_VALUES[j]}' for j in range(1, len(STRATEGY_VALUES)+1)])
    #         # 写入数据到指定列
    #         csvwriter.writerow(metrics_strategy[i, :])
