...
LSIF::LSIF(Forwarder &forwarder, const Name &name)
	: Strategy(forwarder), ProcessNackTraits(this),
	  ...
	  m_nodes(ns3::NodeContainer::GetGlobal()), m_Rth(100.0), m_LET_alpha(5.0)
{
...
LSIF::caculateLET(ns3::Ptr<ns3::Node> sendNode, ns3::Ptr<ns3::Node> revNode) {
    ...
    double let = -(m*p+n*q)+sqrt((pow(p,2)+pow(q,2))*pow(m_Rth,2) - pow(n*p-m*q, 2)) / (pow(p,2)+pow(q,2)+0.0001); // 避免分母为1
    return let;
}
